...
import javaeetutorial.dukesbookstore.session.BookFacade;
...
public class BooksController implements Serializable {
    ...
    public String create() {
        try {
            ...
            JsfUtil.addSuccessMessage(resourceBundle.getString("BookCreated"));
    ...
    public String update() {
        try {
            ...
            JsfUtil.addSuccessMessage(resourceBundle.getString("BookUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, resourceBundle.getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(resourceBundle.getString("BookDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, resourceBundle.getString("PersistenceErrorOccured"));
        }
    }
    ...
    //// creates the list of books for selection in CreateFixed and CreateAuction. Returns items array
    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(ejbFacade.findAll(), true);
    }

    // to get the title to display upon a successful sale listing
    public String getTitle(String ISBN) {
        TypedQuery<Book> query = null;
        em.persist(query);
        query= em.createNamedQuery("Book.findTitlebyIsbn", Book.class);
    ...
    public static class BookControllerConverter implements Converter {
        ...
        @Override
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) { //retrieves the book title & ISBN from selected book
            if (value == null || value.length() == 0) {
                return null;
            }
            
            FacesContext context = FacesContext.getCurrentInstance();
                BooksController controller = (BooksController)context.getApplication()
                        .evaluateExpressionGet(context, "#{booksController}", BooksController.class);
            controller.getBook(getKey(value));
